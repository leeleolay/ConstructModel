...
'''
    ...
    for i in range(0,cellxnum):
        for j in range(0,cellynum):
            for key in cAtomID:
                f.write(
                    str(int(key)+len(cAtomID)*(i*cellynum+j)) + " " + str(1001+i*cellynum+j)+ " 1 " + str(float(cAtomID[key][0])+distOfCell*i) + " " + str(float(cAtomID[key][1])+distOfCell*j) + " " + str(float(cAtomID[key][2])) + "\n"
                )
    # create droplet
